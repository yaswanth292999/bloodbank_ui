...
const PatientTable = () => {
    ...
    async function getAllPatients(){
        ...
        const result=await fetch('http://localhost:3000/v1/patient/getAllPatients')
       ...
       {patientData&&<table>
            ...
            <tbody>
                {patientData.map((patient)=>{
                  return <Patient key={patient.id} patient={patient} bloodbanks={bloodbanks}/>
                })
               }
            </tbody>
...
const [selectedBloodBank,setSelectedBloodBank]=useState<string|null>(null);
...
async function getDonorForBloodGroup(selectedBloodBank:string){
    ...
    const result=await fetch('http://localhost:3000/v1/donor/getDonorForBloodGroup?'+new URLSearchParams({
...
async function registerPatient(donor_id:string|null,patient_id:string,required_units:number,blood_bank:string|null){

    if(!donor_id&&blood_bank&&patientData) return

    const response = await fetch('http://localhost:3000/v1/patient/registerDonorForPatient', {
        method: 'POST',
        body: JSON.stringify({
           donor_id,
           patient_id,
    ...
    return (
        ...
        <td>
            {patientData.blood_bank??<BloodbankSelect blood_banks={bloodbanks} setSelectedBloodBank={setSelectedBloodBank}/>}
        </td>
        <td>
            {patientData.donor_name??<select onChange={(e)=>setSelectedDonor(e.target.value)}><option>Choose donor</option>
            {donors&&donors.map(donor=>{
                return <option key={donor.id} value={donor.id}>
                    {donor.name}
                </option>
            })}
            </select>}
...
function BloodbankSelect({blood_banks,setSelectedBloodBank}:bloodBankSelectProps){
   ...
   {blood_banks&&blood_banks.map((bloodbank)=>{
